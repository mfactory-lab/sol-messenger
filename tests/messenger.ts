...
import type { Messenger } from '../target/types/messenger'
import { decryptCEK, decryptMessage, encryptCEK, encryptMessage, generateCEK } from '../packages/sdk/src/crypto'
...
describe('messenger', () => {
  ...
  const invitee = anchor.web3.Keypair.generate()
  ...
  it('add to channel', async () => {
    ...
    const aca = await program.account.associatedChannelAccount.fetch(inviterAca)
    ...
    await program.methods.addToChannel({ cek: encCEK } as any)
      .accounts({
        channel: channel.publicKey,
